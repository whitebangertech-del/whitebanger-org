...
const benefitIcons = ['ðŸ’»', 'ðŸ…', 'ðŸ“œ'];
const benefitsHtml = data.benefits.map((b, i) => `
<div class="wb-eb-item">
<span class="wb-eb-icon">${benefitIcons[i] || 'âœ”'}</span>
<span>${b}</span>
</div>
`).join('');
...
<span class="wb-checkmark">âœ”</span>
<p>${l}</p>
</div>
`).join('');
...
<span class="wb-doc-icon">âœ”</span>
<span>${e}</span>
</div>
`).join('');
...
<span class="wb-acc-arrow">â–¼</span>
</div>
